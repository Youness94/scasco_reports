...
fetch('/quotes-pie-chart-home')
    .then(response => response.json())
    .then(data => {
        if (data.quotes_status_pending !== undefined && data.quotes_status_processing !== undefined && data.quotes_status_completed !== undefined && data.quotes_status_cancelled !== undefined) {
            var options = {
                series: [
                    data.quotes_status_pending, data.quotes_status_processing,
                    data.quotes_status_completed, data.quotes_status_cancelled,
                ],
                chart: {
                    height: 300,
                    type: 'pie',
                ...
                labels: [
                    "Les devis en attente", "Devis en cours de traitement",
                    "Devis terminés", "Les devis annulés",
                ],
                legend: {
                    position: 'bottom'
                },
                ...
                colors: ["#ff5733", "#3364ff", "#42ff33", "#ff333f"],
            };

            var chart = new ApexCharts(document.getElementById("quotes_chart_pie"), options);
            chart.render();
        }
    })
    .catch(error => {
